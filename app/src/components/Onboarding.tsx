  ...
  return (
    <div className="h-full w-full flex items-center justify-center bg-[#0f0f11] text-white">
      <div className="max-w-md w-full px-6 py-10">
        ...
        <div className="text-base text-white/65 leading-relaxed mb-6">
        ...
        {(cli.claude || cli.codex) ? (
          <div className="rounded-md border border-emerald-400/20 bg-emerald-400/5 px-3.5 py-2.5 text-sm text-white/85">
          ...
          <div className="space-y-3">
            ...
            <input
              ...
              className="w-full bg-white/5 border border-white/10 rounded px-3 py-1.5 text-sm text-white/95 placeholder-white/35 font-mono"
            />
            <button
              type="button"
              onClick={onSave}
              disabled={saving || !key.trim()}
              className="w-full bg-white/10 hover:bg-white/15 disabled:opacity-40 text-sm text-white py-1.5 rounded transition-colors duration-fast"
            >
              {saving ? "saving…" : saved ? "saved — relaunch Vigil" : "save key in Keychain"}
            </button>
            <div className="text-xs text-white/45">Stored securely in macOS Keychain. You can delete it anytime from Keychain Access.</div>
          </div>
        )}
      </div>
    </div>
...
function DetectRow({ label, ok }: { label: string; ok: boolean }) {
  return (
    <div className="flex items-center gap-2 text-base">
      ...
      <span className="ml-auto text-xs text-white/45">{ok ? "detected" : "not found"}</span>
    </div>
  );
}
