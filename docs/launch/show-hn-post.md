...
**What's NOT in v0.1.0d yet:** identity, audit, rate limit, coalesce, and the MCP server for agent introspection (`vigil.identity.whoami`, `vigil.activity.query` over stdio) all ship today. The policy engine (blast-radius enforcement), Redis support, and HTTP/L7 proxy are next. Connection pooling is still 1:1. TLS termination is tracked but not implemented. If you need any of those today, this isn't the release for you.
